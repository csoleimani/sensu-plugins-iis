...
class CurrentFileCacheMemoryUsageMetric < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def accquire_Current_File_Cache_Memory_Usage
    temp_arr = []
    timestamp = Time.now.utc.to_i
    IO.popen("typeperf -sc 1 \"\\W3SVC_W3WP(#{config[:app]})\\Current File Cache Memory Usage\"") { |io| io.each { |line| temp_arr.push(line) } }
  ...
  def run
    values = accquire_Current_File_Cache_Memory_Usage
    metrics = {
      WorkerProcessCache: {
        CurrentFileCacheMemoryUsage: values[0]
      }
    }
