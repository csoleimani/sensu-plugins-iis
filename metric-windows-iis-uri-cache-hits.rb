...
class URICacheHitsMetric < Sensu::Plugin::Metric::CLI::Graphite
  option :scheme,
  ...
  def accquire_URI_Cache_Hits
    temp_arr = []
    timestamp = Time.now.utc.to_i
    IO.popen("typeperf -sc 1 \"\\W3SVC_W3WP(#{config[:app]})\\URI Cache Hits\"") { |io| io.each { |line| temp_arr.push(line) } }
  ...
  def run
    values = accquire_URI_Cache_Hits
    metrics = {
      WorkerProcessCache: {
        URICacheHits: values[0]
      }
    }
